...
Market Narrative Radar keeps open-ended questions inside a fixed evidence contract. The app retrieves passages first, then limits the brief to what those passages can support.
...
The app precomputes source conflict candidates before the final brief. It compares framing by source role:
...
The analysis relay receives these conflict candidates as structured context. It can accept, weaken, or reject them, but it should treat them as framing gaps, not proven factual contradictions.
...
The app accepts open-ended user questions, but it first assigns one text-analysis intent:
...
Provider keys stay outside the browser and are not committed. In a production paid-brief version, checkout would sit in front of the same relay while the evidence contract stays the same.
